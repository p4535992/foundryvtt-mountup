...
import { MOUNT_UP_MODULE_NAME } from "./module/settings.js";
import { initHooks, readyHooks } from "./module/Hooks.js";
export let debugEnabled = 0;
// 0 = none, warnings = 1, debug = 2, all = 3
export const debug = (...args) => {
    if (debugEnabled > 1)
        console.log(`DEBUG:${MOUNT_UP_MODULE_NAME} | `, ...args);
};
export const log = (...args) => console.log(`${MOUNT_UP_MODULE_NAME} | `, ...args);
export const warn = (...args) => {
    if (debugEnabled > 0)
        console.warn(`${MOUNT_UP_MODULE_NAME} | `, ...args);
};
...
Hooks.once('ready', () => {
    ...
    if (!getGame().modules.get('lib-wrapper')?.active && getGame().user?.isGM) {
        ui.notifications?.error(`The '${MOUNT_UP_MODULE_NAME}' module requires to install and activate the 'libWrapper' module.`);
        return;
    }
    if (!getGame().modules.get('token-attacher')?.active && getGame().user?.isGM) {
        ui.notifications?.error(`The '${MOUNT_UP_MODULE_NAME}' module requires to install and activate the 'token-attacher' module.`);
        return;
    }
