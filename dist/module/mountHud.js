...
import { FlagScope, getCanvas } from './settings.js';
...
export class MountHud {
    ...
    static addDismountButton(html, hudToken) {
        ...
        const mount = findTokenById(rider.document.getFlag(FlagScope, Flags.Mount));
    ...
    static buildButton(html, tooltip) {
        const button = $(`<div class="control-icon mount-up" title="${tooltip}"><i class="fas ${SettingsForm.getIconClass()}"></i></div>`);
