...
import { findTokenById, Flags, getTokenShape, riderLock, riderX, riderY } from './utils.js';
/**
...
export class MountManager {
    ...
    static async mountUpHud(hudToken) {
        ...
        for (const riderToken of tokensToCheck) {
            if (riderToken.id != mountToken.id) {
                const mountTokenTmp = findTokenById(riderToken.document.getFlag(FlagScope, Flags.Mount));
                ...
                let riders = mountToken.document.getFlag(FlagScope, Flags.Riders);
                if (riders == undefined) {
                    riders = [];
                }
                ...
                const loc = await this.getRiderInitialLocation(riderToken, mountToken);
    ...
    static async dismount(hudToken) {
        ...
        const mountToken = findTokenById(riderToken.document.getFlag(FlagScope, Flags.Mount));
        // MOD 4535992
    ...
    static async removeAllRiders(hudToken) {
        ...
        const riders = mountToken.document.getFlag(FlagScope, Flags.Riders);
    ...
    static async doRemoveMount(riderToken, mountToken) {
        ...
        for (const riderTmp of riders) {
            const riderTokenTmp = findTokenById(riderTmp);
    ...
    static async restoreRiderSize(riderToken) {
        ...
        if (riders && riders.includes(riderToken.id)) {
            ...
            const origsize = riderToken.document.getFlag(FlagScope, Flags.OrigSize);
    ...
    static async handleTokenDelete(tokenId) {
        if (tokenId) {
            ...
            if (this.isaRider(token.id)) {
                const mount = findTokenById(token.document.getFlag(FlagScope, Flags.Mount));
            ...
            if (this.isaMount(token.id)) {
                const riders = token.document.getFlag(FlagScope, Flags.Riders);
                for (const riderTmp in riders) {
                    const rider = findTokenById(riderTmp);
                    // MOD 4535992 CHECK IF TOKEN IS ALREADY DELETED
                    if (rider) {
                ...
                if (riders && riders.includes(riderToken.id)) {
                    ...
                    if (!riderToken.getFlag(FlagScope, Flags.MountMove)) {
                        if (!getCanvas()
                            ...
                            .includes(riderToken.document.getFlag(FlagScope, Flags.Mount))) {
                            switch (SettingsForm.getRiderLock()) {
                                case riderLock.NoLock:
                                    break;
                                case riderLock.LockLocation:
    ...
    static isInsideTokenBounds(location, token) {
        ...
        return (location.x >= token.x && location.x < token.x + token.w && location.y >= token.y && location.y < token.y + token.h);
    }
    /**
    ...
    static isaMount(tokenId) {
        ...
        if (token) {
            const riders = token.document.getFlag(FlagScope, Flags.Riders);
            return token.document.getFlag(FlagScope, Flags.Riders) != undefined && riders.length > 0;
        }
        else {
            return false;
        }
    ...
    static isRidersMount(riderId, mountId) {
        ...
        return rider.document.getFlag(FlagScope, Flags.Mount) == mount.id;
    }
    // /**
    ...
    //     const offset = { x: mountLocation.x - riderToken.x, y: mountLocation.y - riderToken.y };
    //     if (SettingsForm.getRiderRotate()) {
    ...
    static isAncestor(childId, ancestorId) {
        if (this.isaRider(childId)) {
            ...
            const parent = findTokenById(child.getFlag(FlagScope, Flags.Mount));
