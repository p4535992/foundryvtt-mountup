...
export var injectConfig = {
	inject: function injectConfig(app, html, data, objectOri) {
				...
				case "checkbox":
					injectHtml += `<div class="form-group">
                        <label for="${k}">${v.label || ""}</label>
                            ...
                            onclick="$(this).attr('value', this.checked ? true : false); $(this).attr('checked', this.checked ? true : false);">${notes}
                    </div>`;
					break;
				case "select":
					injectHtml += `<div class="form-group">
		...
		function _bindFilePicker(event) {
			...
			return fp.browse(input.attr.name);
		}
	},
	quickInject: function quickInject(injectData, data) {
		...
		for (const doc of injectData) {
			...
			Hooks.on(`render${doc.documentName}Config`, (app, html) => {
				injectConfig.inject(app, html, newData, undefined);
			});
		}
	},
